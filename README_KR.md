...
<div align="center">
  ...
  <div align="center">
    <a href="https://git.io/typing-svg">
      <img src="https://readme-typing-svg.herokuapp.com?font=Plus+Jakarta+Sans&weight=600&size=35&duration=3000&pause=1000&color=007AFFFF&background=FFFFFF00&center=true&vCenter=true&width=435&lines=Frontend+Technology;Design+Creativity;Business+Insight;My+Color+is+White." alt="Typing SVG" />
    </a>
  </div>
  ...
  <p>
    디자인, 코드, 비즈니스의 간극을 연결하는 <b>프로덕트 마인드 프론트엔드 엔지니어</b>입니다.<br>
    성능을 최적화하고 지속 가능한 프로덕트 성장에 기여하는 <b>확장 가능한 아키텍처</b>를 구축합니다.
  </p>
...
* **성능 최적화:** 커스텀 툴로 스프라이트 기반 이펙트를 최적화하여 에셋 사이즈 **75% 감소(8MB → 2MB)**, 시각적 깜빡임 최소화.
* **아키텍처:** Builder 패턴을 활용한 선언적 UI 아키텍처를 설계하여 구현 시간 **87% 단축(1일 → 1시간 미만)**.
* **결제 모듈:** PC(iframe)와 모바일(리다이렉트) PG 플로우를 통합하는 결제 모듈을 구축하여 **12개 이상의 예외 케이스** 표준화.
* **시스템 설계:** Radix 프리미티브를 활용한 헤드리스 컴포넌트 레이어를 구축하여 멀티 브랜드 디자인 시스템(FC 서울, FC 안양 등) 지원.
...
* **UX 전략:** 유저 페인포인트를 분석하여 중고 명품 마켓 서비스 기획 및 UX 기반 플로우 설계.
* **그로스:** 'Style Challenge' 리워드 설계를 개선하여 유니크 유저 참여 **50% 증가(2,000 → 3,000명)**.
* **마케팅:** Instagram 마케팅 전략을 개편하여 콘텐츠 퀄리티를 높이고 **2만+ 팔로워 증가** 달성.

**→ 프론트엔드 임팩트:** 이 경험이 UI 구축 접근법을 형성했습니다. 현재는 전환 지표와 행동 데이터를 고려하여 컴포넌트 API와 유저 플로우를 설계합니다.
...
* **문제:** 기존 무드 트래커는 텍스트 로그에 의존하여 감정 패턴을 파악하기 어려움.
* **해결:** 감정 데이터를 유기적인 3D 비주얼로 변환. 각 행성의 색상, 텍스처, 움직임이 유저의 감정 상태를 반영.
* **렌더링:** 실시간 유기적 행성 생성을 위한 커스텀 GLSL 셰이더 구현.
* **아키텍처:** Next.js 웹앱과 Chrome 익스텐션 간 로직을 공유하는 `pnpm` 모노레포 설계.
* **인증:** 커스텀 JWT 라이프사이클(15분 액세스 / 30일 리프레시)과 크로스 컨텍스트 세션 동기화를 갖춘 OAuth 2.0 구축.
