...
<div align="center">
  ...
  <p>
    デザイン、コード、ビジネスをつなぐ<b>プロダクト志向のフロントエンドエンジニア</b>です。<br>
...
* **パフォーマンス最適化：** カスタムツールでスプライトベースのエフェクトを最適化し、アセットサイズを**75%削減（8MB → 2MB）**、視覚的なちらつきを最小化。
* **アーキテクチャ：** Builderパターンを用いた宣言的UIアーキテクチャを設計し、実装時間を**87%短縮（1日 → 1時間未満）**。
* **決済モジュール：** PC（iframe）とモバイル（リダイレクト）両方のPGフローに対応した統合決済モジュールを構築し、**12以上の例外ケース**を標準化。
* **システム設計：** Radixプリミティブを活用したヘッドレスコンポーネント層を構築し、マルチブランドデザインシステム（FC Seoul、FC Anyangなど）をサポート。
...
* **マーケティング：** パフォーマンスマーケターと協力して広告クリエイティブを最適化し、会員登録の**CPAを75%削減**。
...
* **レンダリング：** リアルタイムの有機的な惑星生成のためのカスタムGLSLシェーダーを実装。
* **アーキテクチャ：** Next.js WebアプリとChrome拡張機能間でロジックを共有する`pnpm`モノレポを設計。
* **認証：** カスタムJWTライフサイクル（15分アクセス / 30日リフレッシュ）とクロスコンテキストセッション同期を備えたOAuth 2.0を構築。
